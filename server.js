 ...
 * Sessions stored in Supabase — survive server restarts
 */
...
async function ensureAdmin() {
  try {
    ...
    if (!data || !data.length) {
      await sb.from('posusers').insert({ username:'admin', password_hash:hashPw('admin123'), role:'admin', display_name:'Administrator', active:true, created_at:new Date().toISOString() });
      console.log('[Auth] Default admin created: admin / admin123');
    }
  } catch(e) { console.log('[Auth] Note:', e.message); }
}
...
async function dbGetAll() {
  ...
  try {
    ...
    // Auto-migrate old sp_* keys to new names
...
// ── Sessions (stored in Supabase so they survive restarts) ─
...
async function createSession(userId, username, role, displayName) {
  ...
  await sb.from('possessions').insert({ token, user_id: userId, username, role, display_name: displayName, expires_at: expires });
  return token;
}
async function getSession(token) {
  if (!token || !sb || !dbReady) return null;
  try {
...
async function deleteSession(token) {
  try { if (sb && dbReady) await sb.from('possessions').delete().eq('token', token); } catch(e) {}
}
...
async function start() {
  ...
  http.createServer(async (req, res) => {
    ...
    if (url === '/debug') {
      const info = { server:'SwiftPOS', port:PORT, env:IS_LOCAL?'local':'cloud', node_version:process.version, db_connected:dbReady, db_error:dbError||'none', supabase_url_preview:SB_URL?SB_URL.slice(0,40)+'...':'NOT SET', uptime_seconds:Math.floor(process.uptime()), time:new Date().toISOString() };
      if (dbReady) { try { const all = await dbGetAll(); info.db_rows=Object.keys(all).length; info.db_products_count=Array.isArray(all.products)?all.products.length:0; info.db_keys=Object.keys(all); const {data:u} = await sb.from('posusers').select('username,role,active'); info.users=u||[]; } catch(e) { info.err=e.message; } }
      cors(res); res.writeHead(200,{'Content-Type':'application/json'}); return res.end(JSON.stringify(info,null,2));
    }

    if (url === '/debug-keys') {
      try { const {data} = await sb.from('posdata').select('key,updated_at').order('key'); cors(res); res.writeHead(200,{'Content-Type':'application/json'}); return res.end(JSON.stringify({rows:data||[]},null,2)); } catch(e) { return sendJ(res,500,{error:e.message}); }
    }

    if (method === 'GET' && (url === '/' || url === '/pos.html')) {
      if (!fs.existsSync(POS_HTML)) { res.writeHead(404); return res.end('pos.html not found'); }
      cors(res); res.writeHead(200,{'Content-Type':'text/html; charset=utf-8'}); return res.end(fs.readFileSync(POS_HTML));
    }
    ...
    if (url === '/auth/login' && method === 'POST') {
      try {
        ...
        if (IS_LOCAL) {
          const token = makeToken();
          // For local, store a simple in-memory session
          global._localSess = global._localSess || {};
          global._localSess[token] = { username, role:'admin', displayName:username };
    ...
    if (url.startsWith('/users/') && method === 'POST') {
      ...
      try {
        ...
        if (error) throw new Error(error.message);
        if (body.active === false || body.role) await sb.from('possessions').delete().eq('user_id', userId);
        return sendJ(res, 200, { ok:true });
      } catch(e) { return sendJ(res, 500, { ok:false, error:e.message }); }
    }
    ...
    if (url.startsWith('/users/') && method === 'DELETE') {
      ...
      try {
        ...
        await sb.from('possessions').delete().eq('user_id', userId);
        return sendJ(res, 200, { ok:true });
      } catch(e) { return sendJ(res, 500, { ok:false, error:e.message }); }
    }
    ...
    console.log(`\nSwiftPOS running on port ${PORT} | DB: ${dbReady?'Supabase ✓':'connecting...'} | Sessions: Supabase\n`);
